...
# Как подключить аккаунт мессенджера по API
...
Перед началом получите bearer-токен, см. [Авторизацию](../api-reference/authentication.md).
...
Новый аккаунт присутствует в списке со своим UUID. Этот UUID дальше используется для [диалогов](../api-reference/dialogs.md) и для привязки к кампании.
...
- Ответ `401` означает проблему с токеном, а не с аккаунтом — см. [Авторизацию](../api-reference/authentication.md).
- Обрыв на проверке TLS-сертификата не связан ни с токеном, ни с сессией: `api.nexifyneo.com` отдаёт самоподписанный сертификат, см. [Известные ограничения](../api-reference/overview.md).
...
- [Аккаунты в API](../api-reference/accounts.md) — полный перечень операций.
