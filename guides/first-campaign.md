...
После первого касания разговор ведёт AI по вашему сценарию — см. [Чат-флоу](../features/chat-flow.md). Переписку можно читать и продолжать вручную через [операции диалогов](../api-reference/dialogs.md).

Выгрузить базу с результатами обратно: `GET /v2/campaigns/{campaign_id}/contacts/export?format=xlsx`.

## Дальше

- [Кампании и контакты в API](../api-reference/campaigns.md) — полный перечень операций.
- [Диалоги](../api-reference/dialogs.md) — чтение переписки и отправка сообщений.
